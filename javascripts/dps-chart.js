...
//   Each chart is just a <div class="dps-chart" data-values="..."
//   data-ids="..." data-show-icons></div> - see a build page's
...
//
//   data-values / data-ids are parallel comma-separated lists, same
...
//   than re-sorting.
//
//   data-ids - the skill id backing each row (e.g. "deathlyslash"),
//   same id convention as a Skill Setup entry or a gem-priority.js row.
//   It's the AUTHORITATIVE source for both that row's icon
//   (icon-<id>.png, no derivation) AND its display label, resolved via
//   DB_AP_NODE... no - via DB_SKILL_NAMES[id] (skill-names.js), same
//   "id-only, name auto-resolves" pattern as gem-priority.js/
//   skill-setup.js. It's also what gem-dps-tooltip.js matches against
//   to attach a damage-share tooltip to the matching gem card. Include
//   it for every row (basically everything - even a non-skill row like
//   "Bleed" resolves fine, since it has its own DB_SKILL_NAMES entry
//   purely for this purpose - see that file's comment).
//
//   data-labels is an OPTIONAL override of the whole label list -
//   only needed for a genuine one-off id with no DB_SKILL_NAMES entry
//   worth adding (none exist on this site today). When present it's a
//   parallel comma-separated list same length as data-ids/data-values;
//   when absent, every label is derived from data-ids instead. See
//   SiteUtils.resolveChartLabels (site-utils.js) - shared with
//   gem-dps-tooltip.js so both files resolve a chart's labels
//   identically.
//
